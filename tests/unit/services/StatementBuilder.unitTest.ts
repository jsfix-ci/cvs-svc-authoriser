...
describe("StatementBuilder", (): void => {
  ...
  it("should correctly build Statement regardless of setter order", (): void => {
    // reverse order
    const statement: any = new StatementBuilder().setHttpVerb("GET").setStage("stage").setApiId("apiId").setAccountId("accountId").setRegionId("us-east-1").setAction("execute-api:Invoke").setEffect("Allow").build();
  ...
  it("should correctly append resource", (): void => {
    const statement: any = new StatementBuilder().setResource("resource").build();
  ...
  it("should correctly append child resource when resource is present", (): void => {
    const statement: any = new StatementBuilder().setResource("resource").setChildResource("childResource").build();
  ...
  it("should not append child resource when resource is not present", (): void => {
    const statement: any = new StatementBuilder().setResource(null).setChildResource("childResource").build();
