...
describe("getCertificateChain()", () => {
  it("should return an x5c given an existing key ID", async (): Promise<void> => {
    ...
    await expect(getCertificateChain("tenantId", "keyToTheKingdom")).resolves.toEqual(`-----BEGIN CERTIFICATE-----\n${publicKey}\n-----END CERTIFICATE-----`);
  });
  ...
  it("should throw an error if no key matches the given key ID", async (): Promise<void> => {
    ...
    await expect(getCertificateChain("tenantId", "keyToTheKingdom")).rejects.toThrowError("no public key");
  });
